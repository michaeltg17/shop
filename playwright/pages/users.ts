...
export class UserTablePage {
  ...
  /**
   * Navigate to the users page and wait for table to load
   */
   ...
   */
  findRowByData(data: Partial<UserRowData>): Locator {
  ...
  /**
   * Verify a user row exists and is visible
   */
  ...
  /**
   * Verify a complete user row with all data
   */
  async verifyUserRow(firstName: string, data: UserRowData): Promise<void> {
  ...
  /**
   * Open the add dialog and return the UserFormPage
   */
  async openAddDialog(): Promise<UserFormPage> {
...
/**
 * Page Object for the User Add/Edit Dialog
  ...
  async submitAndWaitForResponse(): Promise<void> {
    await Promise.all([
      this.page.waitForResponse(resp =>
        resp.url().includes('/users') && resp.request().method() === 'POST' && resp.status() === 201
      ),
      this.clickAdd()
    ]);
  }
